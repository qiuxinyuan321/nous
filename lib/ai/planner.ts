...
## Hard rules
1. firstAction must be a specific action completable in 15 minutes today (no vague "start planning")
2. ≤ 3 milestones
...
6. 2-4 successCriteria, each verifiable (not "done well")
7. risks max 5, real risks only, no boilerplate
8. deadline null when unsure, do not fabricate dates
...
`
  ...
  const { object } = await generateObject({
    ...
    prompt,
    temperature: 0.3,
    maxOutputTokens: Math.max(provider.maxOutputTokens, 2000),
  })
