...
<h2 id="nous-最近">最近动向 · <code>v0.3.0</code></h2>
...
#### 🏮 Landing 封神
4 个禅意版 Magic UI 组件：<br/>
<code>AuroraInkText</code> 水墨极光字 · <code>InkBorderBeam</code> 流光边框 · <code>ShimmerInkButton</code> 金泥按钮 · <code>NumberTicker</code> 数字滚动

</td>
<td width="25%" valign="top">

#### 📊 工作台 Dashboard
4 张 <b>StatCard</b>：今日聚焦 · 本周想法(较上周) · 本周完成(带 SparkLine) · 连续打卡天数(streak)

</td>
<td width="25%" valign="top">

#### ⌘K 命令面板 v2
基于 <code>cmdk</code> 重写 · 三模式（命令 / 捕获 / 主题）· 分组：想法 5 · 笔记防抖搜索 · 操作 3 · 跳转 8

</td>
<td width="25%" valign="top">

#### 🧪 E2E 回归
Playwright 5 条真回归 + 4 条骨架 · 锁死 bg-clip / conic-gradient / CLS 等脆弱点 · <code>pnpm test:e2e</code> 一键跑

</td>
</tr>
</table>

<sub>实测 · Landing LCP <b>1706→1582ms</b> · Workspace 新 Dashboard LCP <b>1025ms</b> · CLS <b>0</b> · TS / Lint 0 errors · 见 <a href="https://github.com/qiuxinyuan321/nous/releases/tag/v0.3.0">Release Notes</a></sub>
...
全局 **`⌘K`** 唤起 · 基于 [cmdk] 三模式：**命令 / 捕获 / 主题**。

- **命令模式** · 分组 `想法 · 笔记 · 操作 · 跳转`，cmdk 内置 fuzzy 过滤，输入 2 字符触发笔记服务端搜索（防抖 300ms）
- **捕获模式** · 原 Whisper 语音 + 多行 textarea 保留，Enter 落笔，不取名、不分类、不打标签
- **主题子视图** · 三色块预览 + 当前标识 + 实时切换，CSS 变量 inline 防 FOUC
...
<tr>
  <td><b>命令面板</b></td>
  <td>cmdk（Vercel）+ 自定义主题层</td>
  <td>~10KB, 内置 fuzzy + a11y；三模式（命令/捕获/主题）共用同一壳</td>
</tr>
<tr>
  <td><b>E2E</b></td>
  <td>@playwright/test 1.59 · Chromium</td>
  <td>视觉回归锁 bg-clip / conic-gradient / CLS 等脆弱点</td>
</tr>
<tr>
  <td><b>国际化</b></td>
...
- [x] 主题市场（5 款内置）
- [x] **v0.3** 工作台 Dashboard · 4 张 StatCard + SparkLine + streak
- [x] **v0.3** 禅意版 Magic UI（Aurora / Beam / Shimmer / Ticker）
- [x] **v0.3** ⌘K 命令面板 v2（cmdk · 三模式 · 分组搜索）
- [x] **v0.3** Playwright E2E 回归套件（锁死 bg-clip / conic / CLS）
...
- [ ] Auth fixture：放开命令面板那 4 条跳过的 E2E
- [ ] 接回 `SedimentPanel` / `PlanConfirmDialog` / `TaskNotesPopover` 三个待接入组件
...
- **Vercel cmdk** · Raycast 级命令面板底层
- **Prisma** · 让 TypeScript 与 SQL 和解
- **shadcn/ui** · 基础组件哲学借鉴
- **Magic UI** · 动效组件思路启发（Aurora / Border Beam / Shimmer / Number Ticker 墨风改写自同名开源原型）
...
- **v0.3** Dashboard with 4 stat cards · Zen-flavored Magic UI (Aurora / Beam / Shimmer / Ticker) · Raycast-class ⌘K palette (cmdk) · Playwright e2e regression
